...
from core.services.content_ops_service import content_ops_service
...
class HotspotScoreRequest(BaseModel):
    sources: List[str] = ["weibo", "baidu", "toutiao", "bilibili"]
    limit: int = 20
    user_id: Optional[int] = None
    persist: bool = True
...
class OpsPostRequest(BaseModel):
    user_id: Optional[int] = None
    topic_score_id: Optional[int] = None
...
@app.post("/api/ops/topics/score")
async def score_topic(request: TopicScoreRequest):
    """对单个热点/选题进行运营评分"""
    try:
        data = content_ops_service.score_topic(
            {
                "source": request.source,
                "title": request.title,
                "url": request.url,
                "rank": request.rank,
                "hot_value": request.hot_value,
            },
        ...
        logger.error(f"选题评分失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"选题评分失败: {str(e)}")

@app.post("/api/ops/hotspots/score")
async def score_hotspots(request: HotspotScoreRequest):
    """抓取热点榜单并批量评分"""
    try:
        data = content_ops_service.score_hotspots(
        ...
        logger.error(f"热点评分失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"热点评分失败: {str(e)}")

@app.get("/api/ops/topic-scores")
async def list_topic_scores(user_id: Optional[int] = None, limit: int = 50):
    """获取历史选题评分记录"""
    try:
        return {"success": True, "data": content_ops_service.list_topic_scores(user_id=user_id, limit=limit)}
    except Exception as e:
        logger.error(f"获取选题评分记录失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"获取选题评分记录失败: {str(e)}")

@app.post("/api/ops/styles")
async def create_style_profile(request: StyleProfileRequest):
    """创建个人风格档案"""
    try:
        data = content_ops_service.create_style_profile(request.user_id, request.model_dump(exclude={"user_id"}))
        return {"success": True, "data": data}
    except Exception as e:
        logger.error(f"创建风格档案失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"创建风格档案失败: {str(e)}")

@app.get("/api/ops/styles")
async def list_style_profiles(user_id: Optional[int] = None):
    """获取个人风格档案列表"""
    try:
        return {"success": True, "data": content_ops_service.list_style_profiles(user_id=user_id)}
    except Exception as e:
        logger.error(f"获取风格档案失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"获取风格档案失败: {str(e)}")

@app.post("/api/ops/risk-review")
async def review_risk(request: RiskReviewRequest):
    """发布前风险审核"""
    try:
        return {"success": True, "data": content_ops_service.review_risk(request.title, request.content)}
    except Exception as e:
        logger.error(f"风险审核失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"风险审核失败: {str(e)}")

@app.post("/api/ops/posts")
async def create_ops_post(request: OpsPostRequest):
    """记录发布内容，用于后续复盘"""
    try:
        data = content_ops_service.create_post(request.user_id, request.model_dump(exclude={"user_id"}))
        return {"success": True, "data": data}
    except Exception as e:
        logger.error(f"创建复盘帖子失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"创建复盘帖子失败: {str(e)}")

@app.get("/api/ops/posts")
async def list_ops_posts(user_id: Optional[int] = None, limit: int = 50):
    """获取复盘帖子列表"""
    try:
        return {"success": True, "data": content_ops_service.list_posts(user_id=user_id, limit=limit)}
    except Exception as e:
        logger.error(f"获取复盘帖子失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"获取复盘帖子失败: {str(e)}")

@app.post("/api/ops/posts/{post_id}/metrics")
async def record_post_metric(post_id: int, request: PostMetricRequest):
    """录入帖子效果数据"""
    try:
        data = content_ops_service.record_post_metric(post_id, request.model_dump())
        return {"success": True, "data": data}
    except Exception as e:
        logger.error(f"录入帖子指标失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"录入帖子指标失败: {str(e)}")

@app.get("/api/ops/review")
async def analyze_ops_posts(user_id: Optional[int] = None, limit: int = 100):
    """汇总复盘结果，输出高表现内容和标签"""
    try:
        return {"success": True, "data": content_ops_service.analyze_posts(user_id=user_id, limit=limit)}
    except Exception as e:
        logger.error(f"复盘分析失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"复盘分析失败: {str(e)}")
